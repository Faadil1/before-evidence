...
type SessionRow={
  ...
  updated_confidence:number|null; proposed_next_experiment:string|null; audit:Session["audit"]|null; audit_status:Session["auditStatus"]|null; audit_error:string|null;
...
function rowToSession(row:SessionRow):Session{return {sessionId:row.session_id,scenarioId:row.scenario_id,scenarioVersion:row.scenario_version,status:row.status,commitment:row.commitment??undefined,lockedAt:iso(row.locked_at),evidenceVersion:row.evidence_version??undefined,evidenceRevealedAt:iso(row.evidence_revealed_at),interpretation:row.interpretation??undefined,updatedConfidence:row.updated_confidence??undefined,proposedNextExperiment:row.proposed_next_experiment??undefined,audit:row.audit??undefined,auditStatus:row.audit_status??"idle",auditError:row.audit_error??undefined,createdAt:iso(row.created_at),updatedAt:iso(row.updated_at)};}
...
)`);await pool().query("alter table sessions add column if not exists audit_status text not null default 'idle'");await pool().query("alter table sessions add column if not exists audit_error text");}
