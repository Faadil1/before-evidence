...
describe("persistent store contract",()=>{
 it("creates independent session records and locks atomically once",async()=>{
  ...
  const locked=await lockSession(a.sessionId,commitment);expect(locked.status).toBe("locked");if(locked.status==="locked"){expect(locked.session.recordFingerprint).toMatch(/^sha256:/);expect(locked.session.recordSchemaVersion).toBe("before-evidence-record-1");}
